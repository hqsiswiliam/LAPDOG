...
@dataclass
class Seq2SeqLMRLOutput(ModelOutput):
    loss: Optional[torch.FloatTensor] = None
    loss_no_reduction: Optional[torch.FloatTensor] = None
...
class T5ForConditionalGeneration(T5PreTrainedModel):
    ...
    def forward(
        ...
        loss = None
        loss_no_reduction = None
        if labels is not None:
            loss_fct = CrossEntropyLoss(ignore_index=-100)
            loss = loss_fct(lm_logits.view(-1, lm_logits.size(-1)), labels.view(-1))
            loss_fct = CrossEntropyLoss(ignore_index=-100, reduction='none')
            loss_no_reduction = loss_fct(lm_logits.view(-1, lm_logits.size(-1)), labels.view(-1)).view(labels.size(0),-1)
        ...
        return Seq2SeqLMRLOutput(
            loss=loss,
            loss_no_reduction=loss_no_reduction,
            logits=lm_logits,
            past_key_values=decoder_outputs.past_key_values,
            decoder_hidden_states=decoder_outputs.hidden_states,
            decoder_attentions=decoder_outputs.attentions,
