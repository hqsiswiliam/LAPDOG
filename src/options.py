...
class Options:
    ...
    def initialize_parser(self):
        ...
        self.parser.add_argument(
            "--add_random_retrieval",
            type=float,
            default=0,
            help="add random retrieval to training, the value is bounded from 0 to 1",
        )
        ...
        self.parser.add_argument(
            "--retriever_from", type=str, help="used in convai2 dataset, retrieved using persona only or all",
            default="all", choices=['all', 'persona', 'context', 'forward', 'persona_rand']
        )

        self.parser.add_argument(
            "--story_label", type=str, help="the string before story, such as 'story: ..., persona: ..., context: ...'",
            default="", choices=["", "S:"]
        )
    ...
    def add_optim_options(self):
        ...
        self.parser.add_argument("--fix_reader", action="store_true", help="fix reader, not to train reader")
        ...
        self.parser.add_argument(
            "--add_null_retrieval",
            action="store_true",
            help="whether to add null in retrieval results",
        )
    ...
    def add_modeling_options(self):
        ...
        self.parser.add_argument(
            ...
            type=int,
            default=200,
            help="maximum number of tokens in input text segments (concatenated story+context). Inputs longer than this will be truncated.",
        )
        self.parser.add_argument(
            "--target_maxlength",
            type=int,
        ...
        self.parser.add_argument("--n_context", type=int, default=1, help="number of top k stories to pass to reader")
        ...
        self.parser.add_argument(
            "--scale_rouge",
            action="store_true",
            help='scale metric-guided rouge score by 100x',
        )

        self.parser.add_argument(
            "--reader_rl_learning",
            action="store_true",
            help="whether to do rl tuning on reader",
        )
        ...
        self.parser.add_argument(
            ...
            type=str,
            default=None,
            help="scale factors for metrics",
        )
        ...
        self.parser.add_argument(
            "--gold_score_mode",
            type=str,
            choices=["evalnormsum", "loop", "ppmean", "emdr", "pdist", "adist", "bleudist", "bleudist_pdist", "f1rougebleudist"],
            default="ppmean",
        ...
        self.parser.add_argument(
            "--encoder_format",
            type=str,
            default="{query} story title: {title} story: {text}",
        ...
        self.parser.add_argument(
            ...
            type=str,
            default='qa',
            choices=["qa", "causal_chat"],
            help="qa option here is specifically for PersonaChat, where we pre-process training and evaluation data",
        )
        ...
        self.parser.add_argument(
            "--qa_prompt_format",
            type=str,
            default="dialog with R's persona: {question} R: <extra_id_0>",
            help="How to format PersonaChat dataset as input prompts when using --task qa",
        )
    ...
    def parse(self):
        ...
        assert opt.qa_prompt_format in ["question: {question} answer: <extra_id_0>",
                                        "dialog with R's persona: {question} R: <extra_id_0>"]
        if opt.qa_prompt_format == "question: {question} answer: <extra_id_0>" \
                and 'convai' in ' '.join(opt.train_data):
            answer = input("Use old prompt as `question: {question} answer: <extra_id_0>`. "
                           "Continue? (y/n)")
        ...
        if opt.reader_causallm is not None:
            if opt.encoder_format == self.parser.get_default('encoder_format'):
                opt.encoder_format = "title: {title} story: {text} {query}"
                import warnings
                warnings.warn("The encoder_format is override with title: {title} story: {text} {query}")
